...
export function Carrito() {
  const { items, lineas, unidades, total, cambiarCantidad, quitar, vaciar, listo } = useTienda();
  ...
  if (lineas.length === 0) {
    return (
      <div className="mt-10 rounded-2xl border border-borde bg-superficie p-10 text-center">
        <p className="text-tenue">Todavía no agregaste nada.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <Link
            href="/armar"
            className="inline-flex rounded-md bg-acento px-7 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90"
          >
            Armar mi PC
          </Link>
          <Link
            href="/productos"
            className="inline-flex rounded-md border border-borde px-7 py-3 text-sm font-semibold transition-colors hover:border-acento"
          >
            Ver productos
          </Link>
        </div>
  ...
  const href = linkPedido(lineas, total);
    ...
    <>
        ...
        {lineas.map((linea, i) => (
          <li key={i} className="flex gap-4 p-4">
            <div className="flex min-w-0 flex-1 flex-col">
              {linea.slug ? (
                <Link href={`/productos/${linea.slug}`} className="text-sm font-medium hover:text-acento">
                  {linea.nombre}
                </Link>
              ) : (
                <span className="text-sm font-medium">{linea.nombre}</span>
              )}
              {linea.detalle && <p className="mt-0.5 text-xs leading-relaxed text-tenue">{linea.detalle}</p>}
              <p className="mt-0.5 text-xs text-tenue">{precio(linea.unitario)} c/u</p>
      ...
      <div className="mt-6 rounded-2xl border border-borde bg-superficie p-6">
        ...
        <a
          ...
          className="mt-5 flex items-center justify-center gap-2.5 rounded-md bg-[#25D366] px-7 py-4 text-sm font-semibold text-black transition-opacity hover:opacity-90"
        >
        ...
        <p className="mt-3 text-center text-xs leading-relaxed text-tenue">
          Se abre WhatsApp con tu pedido ya escrito. Coordinás pago y envío directo por el chat.
        </p>
        ...
        <div className="mt-5 flex justify-between text-xs">
          ...
          {items.length > 0 && (
            <button type="button" onClick={vaciar} className="text-tenue hover:text-foreground">
              Vaciar pedido
            </button>
          )}
        </div>
